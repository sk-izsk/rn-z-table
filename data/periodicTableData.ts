...
export const MAIN_GRID_CELLS: PeriodicGridCell[] = Array.from(
  { length: 7 },
  (_rowEntry, rowIndex) =>
    Array.from({ length: 18 }, (_columnEntry, columnIndex): PeriodicGridCell => {
      const row = rowIndex + 1
      const col = columnIndex + 1
      const element = byPosition.get(`${row},${col}`)
